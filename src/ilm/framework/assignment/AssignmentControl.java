...
public final class AssignmentControl implements IAssignment,
		IAssignmentOperator, IlmProtocol {
	...
	public AssignmentControl(SystemConfig config, ICommunication comm,
			DomainModel model, DomainConverter converter) {
		_config = config;
		_comm = comm;
		_model = model;
	...
	private void initAssignments() {
		...
		try {
			numberOfPackages = Integer.parseInt(_config
					.getValue(IlmProtocol.NUMBER_OF_PACKAGES));
		...
		if (numberOfPackages > 0) {
			for (int i = 0; i < numberOfPackages; i++) {
				String packageFileName = _config
						.getValue(IlmProtocol.ASSIGNMENT_PACKAGE_PATH + "_" + i);
				getConfigFromMetadataFile(loadMetadataFile(packageFileName));
				_assignmentList
						.addAll(createAssignments(loadAssignmentFiles(packageFileName)));
			}
		} else {
	...
	private Vector createAssignments(Vector stringList) {
		...
		for (int i = 0; i < stringList.size(); i++) {
			Assignment a = parser.convertStringToAssignment(_converter,
					(String) stringList.get(i));
			assignmentList.add(a);
			if (a.getExpectedAnswer() == null
					|| a.getExpectedAnswer().getList().size() < 1
					|| !a.getInitialState().equals(a.getCurrentState())) {
				parser.setAssignmentModulesData(_converter,
						(String) stringList.get(i), _moduleList, i + n);
			} else {
		...
		while (moduleIterator.hasNext()) {
			...
			if (_moduleList.get(key) instanceof AssignmentModule) {
				...
				if (((AssignmentModule) _moduleList.get(key)).getObserverType() != AssignmentModule.OBJECT_OBSERVER) {
					((AssignmentModule) _moduleList.get(key))
							.setDomainModel(_model);
					((AssignmentModule) _moduleList.get(key))
							.setActionObservers(_moduleList.values());
				}
				((AssignmentModule) _moduleList.get(key)).setState(assignment
						.getCurrentState());
			}
		}
	}
	...
	private Vector loadAssignmentFiles(String packageFileName) {
		...
		Vector assignmentFileList = parser
				.getAssignmentFileList(metadataFileContent);
		HashMap metadata = parser.convertStringToMap(metadataFileContent,
				IlmProtocol.METADATA_LIST_NODE);
		try {
			Vector assignmentList = _comm.readAssignmentFiles(packageFileName,
					assignmentFileList);
	...
	public ZipFile saveAssignmentPackage(Vector assignmentList, String fileName) {
		AssignmentParser parser = new AssignmentParser();
		String metadataFileContent = parser.createMetadataFileContent(
				assignmentList, _config.toString());
		Vector assignmentNameList = parser
				.getAssignmentFileList(metadataFileContent);
		...
		for (int i = 0; i < assignmentList.size(); i++) {
			...
			if (((Assignment) assignmentList.get(i)).getExpectedAnswer() == null
					|| ((Assignment) assignmentList.get(i)).getExpectedAnswer()
							.getList().size() < 1
					|| !((Assignment) assignmentList.get(i)).getInitialState()
							.equals(((Assignment) assignmentList.get(i))
									.getCurrentState())) {
				assignmentContent = parser.getAssignmentModulesData(_converter,
						assignmentContent, _moduleList, i);
			}
			assignmentContentList.add(assignmentContent);
		}
		try {
			return _comm.writeAssignmentPackage(fileName, metadataFileContent,
					null, null, assignmentNameList, assignmentContentList);
		} catch (IOException e) {
			e.printStackTrace();
		}
		return null;
	...
	public int openAssignmentPackage(String fileName) {
		...
		_assignmentList
				.addAll(createAssignments(loadAssignmentFiles(fileName)));
	...
	public float getEvaluation() {
		return ((AutomaticCheckingModule) _moduleList
				.get(IlmProtocol.AUTO_CHECKING_MODULE_NAME)).getEvaluation();
	}

	public String getAnswer() {
		return ((AutomaticCheckingModule) _moduleList
				.get(IlmProtocol.AUTO_CHECKING_MODULE_NAME)).getAnswer();
	}
