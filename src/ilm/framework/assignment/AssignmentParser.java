...
final class AssignmentParser {
	...
	public Assignment convertStringToAssignment(DomainConverter converter,
		...
		HashMap config = convertStringToMap(assignmentString,
				IlmProtocol.CONFIG_LIST_NODE);
		HashMap metadata = convertStringToMap(assignmentString,
				IlmProtocol.METADATA_LIST_NODE);
		Assignment assignment = new Assignment(proposition, initialState,
	...
	public String getProposition(String assignmentString) {
		int startIndex = assignmentString.indexOf("<"
				+ IlmProtocol.ASSIGNMENT_PROPOSITION + ">");
		int endIndex = assignmentString.indexOf("</"
				+ IlmProtocol.ASSIGNMENT_PROPOSITION + ">");
		return assignmentString.substring(startIndex + 2
				+ IlmProtocol.ASSIGNMENT_PROPOSITION.length(), endIndex);
	}

	public AssignmentState getState(DomainConverter converter,
			String assignmentString, String nodeName) {
		AssignmentState state = new AssignmentState();
		int startIndex = assignmentString.indexOf("<" + nodeName + ">") + 2
				+ nodeName.length();
		int endIndex = assignmentString.indexOf("</" + nodeName + ">");
		if (startIndex == -1 | endIndex == -1) {
			return state;
		}
		String objectListString = assignmentString.substring(startIndex,
				endIndex);
	...
	public HashMap convertStringToMap(String xmlContent, String nodeName) {
		...
		String listString = xmlContent.substring(startIndex, endIndex + 3
				+ listNodeLength);
	 ...
	 * @return
	 */
	public String convertAssignmentToString(DomainConverter converter,
			Assignment assignment) {
		String string = "<" + IlmProtocol.ASSIGNMENT_FILE_NODE + ">";
		string += "<" + IlmProtocol.ASSIGNMENT_PROPOSITION + ">"
				+ assignment.getProposition() + "</"
				+ IlmProtocol.ASSIGNMENT_PROPOSITION + ">";
		string += "<"
				+ IlmProtocol.ASSIGNMENT_INITIAL_NODE
				+ ">"
				+ converter.convertObjectToString(assignment.getInitialState()
						.getList()) + "</"
				+ IlmProtocol.ASSIGNMENT_INITIAL_NODE + ">";
		if (assignment.getCurrentState().getList().size() > 0) {
			string += "<"
					+ IlmProtocol.ASSIGNMENT_CURRENT_NODE
					+ ">"
					+ converter.convertObjectToString(assignment
							.getCurrentState().getList()) + "</"
					+ IlmProtocol.ASSIGNMENT_CURRENT_NODE + ">";
		} else {
	...
	public Vector getAssignmentFileList(String metadataFileContent) {
		...
		int startIndex = metadataFileContent.indexOf("<"
				+ IlmProtocol.FILE_LIST_NODE + ">");
		int endIndex = metadataFileContent.indexOf("</"
				+ IlmProtocol.FILE_LIST_NODE + ">");
		String fileListString = metadataFileContent.substring(startIndex,
				endIndex + 3 + listLength);
		Vector assignmentFileList = new Vector();
		int fileLength = IlmProtocol.ASSIGNMENT_FILE_NODE.length();
		endIndex = 0;
		do {
			startIndex = fileListString.indexOf("<"
					+ IlmProtocol.ASSIGNMENT_FILE_NODE + ">", endIndex)
					+ 2 + fileLength;
			endIndex = fileListString.indexOf("</"
					+ IlmProtocol.ASSIGNMENT_FILE_NODE + ">", startIndex);
			assignmentFileList.add(fileListString.substring(startIndex,
					endIndex));
		} while (endIndex < fileListString.lastIndexOf("</"
	...
	public Vector mergeMetadata(Vector assignmentList, HashMap metadata) {
		...
		for (int i = 0; i < assignmentList.size(); i++) {
			...
			metadataIndex = assignment.indexOf("<"
					+ IlmProtocol.METADATA_LIST_NODE + ">");
			if (metadataIndex == -1) {
				metadataString = "<" + IlmProtocol.METADATA_LIST_NODE + ">"
						+ metadataString + "</"
						+ IlmProtocol.METADATA_LIST_NODE + ">";
				mergedAssignment = assignment.substring(
						0,
	...
	public String createMetadataFileContent(Vector list, String config) {
		...
		for (int i = 0; i < list.size(); i++) {
			...
			string += "<" + IlmProtocol.ASSIGNMENT_FILE_NODE + ">"
					+ a.getName() + "</" + IlmProtocol.ASSIGNMENT_FILE_NODE
					+ ">";
		}
		string += "</" + IlmProtocol.FILE_LIST_NODE + "><"
	 ...
	 * @param availableList
	 */
	public void setAssignmentModulesData(DomainConverter converter,
			String assignmentString, HashMap availableList, int assignmentIndex) {
		int startIndex = assignmentString.indexOf("<"
				+ IlmProtocol.ASSIGNMENT_MODULES_NODE + ">");
		int endIndex = assignmentString.indexOf("</"
				+ IlmProtocol.ASSIGNMENT_MODULES_NODE + ">");
		if (startIndex == -1 || endIndex == -1) {
			return;
		...
		while (availableListIterator.hasNext()) {
			...
			if (endIndex != -1) {
				moduleString = moduleListString.substring(
						startIndex + key.length() + 2, endIndex);
			} else {
				moduleString = "";
			}
			if (availableList.get(key) instanceof AssignmentModule) {
				((AssignmentModule) availableList.get(key))
						.setContentFromString(converter, assignmentIndex,
								moduleString);
			}
		}
	}

	public String getAssignmentModulesData(DomainConverter converter,
			String assignmentString, HashMap availableList, int assignmentIndex) {
		...
		while (availableListIterator.hasNext()) {
			String key = (String) availableListIterator.next();
			if (availableList.get(key) instanceof AssignmentModule) {
				string += ((AssignmentModule) availableList.get(key))
						.getStringContent(converter, assignmentIndex);
			}
		}
		string += "</" + IlmProtocol.ASSIGNMENT_MODULES_NODE + ">";
		int index = assignmentString.lastIndexOf("</"
				+ IlmProtocol.ASSIGNMENT_FILE_NODE + ">");
		return assignmentString.substring(0, index) + string
				+ assignmentString.substring(index);
	}
}
