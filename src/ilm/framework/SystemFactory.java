...
import usp.ime.line.ivprog.view.IVPBaseGUI;
...
public abstract class SystemFactory {
	...
	public abstract DomainGUI createDomainGUI(SystemConfig config,
			DomainModel domainModel);

	public BaseGUI createBaseGUI(SystemConfig config, IAssignment assignment,
			SystemFactory factory) {
		BaseGUI gui = new IVPBaseGUI();
		gui.setComponents(config, assignment, factory);
		return gui;
	}

	public abstract AuthoringGUI createAuthoringGUI(DomainGUI domainGUI,
			String proposition, AssignmentState initial,
			AssignmentState current, AssignmentState expected, HashMap config,
			HashMap metadata);
	...
	public final AssignmentControl createAssignmentControl(SystemConfig config,
		...
		AssignmentControl assignControl = new AssignmentControl(config, comm,
				model, converter);
