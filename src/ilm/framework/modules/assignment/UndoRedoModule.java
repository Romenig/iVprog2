...
public class UndoRedoModule extends AssignmentModule {
	...
	public void undo() {
		DomainAction action = (DomainAction) ((Stack) _undoStack
				.get(_assignmentIndex)).pop();
		((Stack) _redoStack.get(_assignmentIndex)).push(action);
		action.undo();
	}

	public void redo() {
		DomainAction action = (DomainAction) ((Stack) _redoStack
				.get(_assignmentIndex)).pop();
		action.setRedo(true);
	...
	public void update(Observable o, Object arg) {
		if (o instanceof DomainAction) {
			...
			if (action.isUndo()) {
				...
				((Stack) _undoStack.get(_assignmentIndex))
						.push((DomainAction) action.clone());
				setChanged();
				notifyObservers();
			}
		}
	...
	private String getActions(String string, String moduleContent) {
		int startIndex = moduleContent.indexOf("<" + string + ">") + 2
				+ string.length();
	...
	public String getStringContent(DomainConverter converter, int index) {
		...
		if (((Stack) _undoStack.get(index)).size() > 0) {
			string += "<undostack>";
			string += converter.convertActionToString(((Vector) _undoStack
					.get(index)));
			string += "</undostack>";
		}
	...
	public void setState(AssignmentState state) {
		for (int i = 0; i < ((Stack) _undoStack.get(_undoStack.size() - 1))
				.size(); i++) {
			DomainAction action = (DomainAction) ((Stack) _undoStack
					.get(_undoStack.size() - 1)).get(i);
			action.setState(state);
		}
		for (int i = 0; i < ((Stack) _redoStack.get(_redoStack.size() - 1))
				.size(); i++) {
			DomainAction action = (DomainAction) ((Stack) _redoStack
					.get(_redoStack.size() - 1)).get(i);
			action.setState(state);
		}
	}
