...
public class JTaskPaneGroup extends JPanel implements
  ...
  public Component add(JComponent c, String toolTip){
	  if(c instanceof JPanel){
		  ((JPanel)c).setToolTipText(toolTip);
	  }
	  add(c);
	  return c;
  }
  
  public Component add(Action action, String toolTip) {
	Component c = ((TaskPaneGroupUI)ui).createAction(action);
	if(c instanceof JButton){
		((JButton) c).setToolTipText(toolTip);
	}
	add(c);
	return c;
  }
